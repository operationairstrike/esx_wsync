...
-- Access ESX object using the updated method
...
ESX.RegisterCommand('weather', 'admin', function(xPlayer, args, showError)
    ...
    local weatherType = string.upper(args.weatherType)

    for _, type in ipairs(AvailableWeatherTypes) do
        if type == weatherType then
    ...
    if validWeatherType then
        TriggerClientEvent('esx:showNotification', xPlayer.source, 'Weather will change to: ~y~' .. string.lower(weatherType) .. "~s~.")
